...
def get_supabase_connection():
    ...
    # Database connection: postgresql://postgres:[project-ref]:5432/postgres
    if "supabase.co" in SUPABASE_URL:
        # Extract project reference from URL
        project_ref = SUPABASE_URL.split("//")[1].split(".")[0]
        db_url = f"postgresql://postgres.{project_ref}:{SUPABASE_SERVICE_KEY}@aws-0-{project_ref}.pooler.supabase.com:6543/postgres"
    else:
...
def get_supabase_schema() -> Dict[str, List[Dict[str, Any]]]:
    """Extract schema information from Supabase."""
    conn = get_supabase_connection()
    if not conn:
        print("Could not connect to Supabase. Using sample schema.")
        return get_sample_schema()
...
def get_sample_schema() -> Dict[str, List[Dict[str, Any]]]:
...
def main():
    """Main function to generate Dart models."""
    print("Generating Dart models from Supabase schema...")
    ...
    if not SUPABASE_URL or not SUPABASE_SERVICE_KEY:
        ...
        print("Using sample schema for demonstration.")
